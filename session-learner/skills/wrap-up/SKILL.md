...
Reflect on the current session and *diverge*: surface what's worth noticing, then list candidate take-away topics. When a watch-out needs a concrete detail, use `Read`/`Grep`/`Glob` to inspect files referenced this session. Display everything in the terminal. Do NOT write any files.
...
1–4 items. Each must be a pitfall hit, a footgun, a fragile assumption, or a non-obvious thing worth remembering. Each must also trace to a real moment in this session (a correction, a bug, a surprising constraint). Skip generic advice. If an item cannot be tied to something that actually happened, drop it.
...
If nothing substantial happened (no real work, debugging, or decisions), output exactly this and stop. Do not include watch-outs or a topic list:
