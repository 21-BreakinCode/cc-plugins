...
description: Use to toggle the adhd-review reply style for the current session. It turns ADHD-friendly, action-first, blocker-before-FYI reply shaping on or off. Triggers on "adhd mode", "review-ready output", "stop adhd mode" / "normal mode", or questions about the always-on flag or subagent safety.
---

# adhd-review-mode

Toggle the `adhd-review` reply style for **this session**, the human-facing thread.

## Turn it ON

When the user asks for adhd mode / review-ready output:

1. Read the style at `${CLAUDE_PLUGIN_ROOT}/scripts/session-rules.md`.
...
The hook injects the style into the **main session only**. The plugin ships no output style,
so the `outputStyle` setting has no part in it.
...
- `SessionStart` fires for the **main session only**. Subagents do not inherit its injected
  context.
- The plugin registers no `SubagentStart` or `SubagentStop` hook, so nothing runs for a
  subagent.

By contrast, `CLAUDE.md` and `~/.claude/rules/*.md` **are** inherited by subagents. This is
exactly why this shaping ships as a **SessionStart hook**, not as CLAUDE.md
...
hard mechanism. It matters for a fork, because a fork copies the whole conversation, rules
included.

## Verify it works

1. **Style applies (main thread).** In a fresh session (or after this skill), replies
...
4. **Disable is a clean no-op.** With `CLAUDE_ADHD_REVIEW=0`, `scripts/session-start.sh` exits 0
   and emits nothing. By default it emits the style body from `scripts/session-rules.md`.
